...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        List<Integer> taskTimeList = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                /*.map(days -> ((LocalDate.now().getYear() - days.getCreated().getYear())*365) +
                        ((LocalDate.now().getMonthValue() - days.getCreated().getMonthValue())*30) +
                        ((LocalDate.now().getDayOfMonth() - days.getCreated().getDayOfMonth())))*/
